...
#### 验收标准
1. WHEN 配置检查项 THEN 配置SHALL包含：检查周期、检查项名称、所需采集功能列表、数据范围模式
2. WHEN 配置检查项 THEN 每个采集功能SHALL可配置具体参数（如指标类型、日志路径/模块、匹配模式）
3. WHEN 配置检查项 THEN 支持为不同检查项设置不同检查周期（1min~60min，分钟级精度），特殊检查项可设置更高下限
4. WHEN 配置检查项 THEN 支持启用/禁用单个检查项而不影响其他项
5. WHEN 配置检查项 THEN 支持设置数据范围模式："仅本地"、"集群补充"、"自动选择"
...
### 需求8：节点资源使用监控（节点本地独有）
**用户故事**：作为集群管理员，我需要监控节点本地独有的资源使用情况，避免与kubelet支持的重叠
...
6. WHEN kubelet已提供基础资源使用率 THEN 系统SHALL跳过对应检查项避免重复，仅保留节点本地独有指标

### 需求9：K8s核心组件健康检查（节点本地视角）
**用户故事**：作为SRE工程师，我需要从节点本地视角检查K8s组件健康，补充kubelet无法提供的信息
...
6. WHEN kubelet已提供组件基础健康状态 THEN 系统SHALL跳过对应检查项避免重复，仅保留节点本地独有细节

### 需求10：容器运行时状态监控（节点本地细节）
**用户故事**：作为运维人员，我需要监控节点本地容器运行时的细节状态，补充kubelet无法获取的信息
...
6. WHEN kubelet已提供容器基础状态 THEN 系统SHALL跳过对应检查项避免重复，仅保留节点本地独有细节
...
6. WHEN 执行网络检查时 THEN 系统SHALL限制检查周期在1min~60min范围内可配置，避免频繁网络探测，仅检查本地网络栈状态
...
2. WHEN 工具运行时 THEN 系统SHALL限制内存使用：常规运行状态不超过100MB，数据采集高峰期可临时扩展到150MB
3. WHEN 节点资源紧张时 THEN 系统SHALL自动降低检查频率或暂停非关键检查项
4. WHEN 配置检查项时 THEN 系统SHALL支持为每个检查项设置检查周期（1min~60min，分钟级精度）
5. WHEN 检查项执行超时 THEN 系统SHALL强制终止并记录超时事件
6. WHEN 工具异常退出 THEN 系统SHALL优雅退出并释放所有资源

### 需求17：kubelet已有采集数据去重机制
**用户故事**：作为系统管理员，我需要确保节点诊断工具专注于节点本地独有数据，避免与集群级监控重复

#### 验收标准
1. WHEN 配置检查项时 THEN 系统SHALL通过数据范围模式统一控制："仅本地"、"集群补充"、"自动选择"
2. WHEN 选择"仅本地"模式时 THEN 系统SHALL仅采集节点本地独有指标：CPU throttling详情、内存碎片、磁盘SMART状态、网络接口错误、kubelet本地日志、容器运行时socket响应时间等
3. WHEN 选择"集群补充"模式时 THEN 系统SHALL仅采集集群监控无法提供的细节数据：进程级资源使用、内核日志、本地配置文件状态、证书有效期等
4. WHEN 选择"自动选择"模式时 THEN 系统SHALL基于预定义的数据类型映射自动判断是否为节点本地独有数据
5. WHEN 检查项被标记为集群级数据时 THEN 系统SHALL在数据中标注跳过原因"集群级监控已覆盖"
6. WHEN 配置检查项时 THEN 系统SHALL提供数据类型白名单，明确标识哪些数据类型属于节点本地独有
7. WHEN 执行节点本地检查时 THEN 系统SHALL确保所有检查命令在节点本地执行，不依赖apiserver或集群级服务
8. WHEN 检查项配置为采集集群级数据时 THEN 系统SHALL拒绝执行并提示用户使用集群级监控工具